...
class ClientUser {
  ...
  static async create(userData) {
    ...
    // 保存用户数据
    pipeline.set(`${clientConfig.redis.keyPrefix.user}${user.id}`, JSON.stringify(user))

    // 创建邮箱索引
    pipeline.set(`${clientConfig.redis.keyPrefix.userEmail}${email}`, user.id)
  ...
  async update(updates) {
    ...
    // 保存到Redis
    await redis.set(`${clientConfig.redis.keyPrefix.user}${this.id}`, JSON.stringify(this))
  ...
  async generateResetToken() {
    ...
    // 保存令牌（1小时过期）
    await redis.setex(`${clientConfig.redis.keyPrefix.resetToken}${resetToken}`, 3600, this.id)
  ...
  async updateUsage(tokens, requests = 1) {
    ...
    // 更新月度使用量
    await redis.hincrby(`${clientConfig.redis.keyPrefix.usage}monthly:${this.id}`, monthKey, tokens)

    // 更新总使用量
    await redis.hincrby(`${clientConfig.redis.keyPrefix.usage}total:${this.id}`, 'tokens', tokens)
    await redis.hincrby(
  ...
  async getStatistics() {
    ...
    // 获取总使用量
    const totalStats = await redis.hgetall(`${clientConfig.redis.keyPrefix.usage}total:${this.id}`)
